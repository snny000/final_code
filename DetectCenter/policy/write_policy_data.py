...
director_rule_models = {
    ...
    'picture_file': DirectorPictureRule,
...
def get_rule_fields(module_name):
    ...
    elif module_name == 'picture_file':  # 图片筛选回传
        result_set = ('rule_id', 'filesize_minsize', 'filesize_maxsize', 'risk')
...
def process_policy(module_name):
    try:
        if not config.const.UPLOAD_BUSINESS_DISPOSAL:
            ...
            return
        try:
            policy_flag = int(cf.get('policy_flag', module_name))
        except:
            policy_flag = 1
        ...
        with open(file_path, 'wb') as f_handler:
            ...
            elif module_name in ['keyword_file', 'encryption_file', 'compress_file', 'picture_file']:
            ...
            for d_id in id_device_dict.values():
                ...
                if query_data.exists():
                    ...
                    elif module_name in ['encryption_file', 'picture_file']:    # 加密规则或图片规则，修改config的组织方式
                        for data in rules:
