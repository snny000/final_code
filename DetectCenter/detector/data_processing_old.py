    ...
    try:
        ...
        if serializer.is_valid():
            ...
            if config.const.UPLOAD_BUSINESS_DISPOSAL:
                handle_data_type = 'register(detector)'
                file_dir = os.path.join(config.const.DISPOSAL_DIR, 'detector')
                file_name = 'detector' + str(int(time.time())) + '_' + str(1)
...
def check_re_register_detector(request):
    ...
    #     return common.detector_message_response(403, '检测器没有提交过注册请求', '检测器从未注册过',
...
def re_register(request):
    try:
        ...
        log_str = '%s %s %s %s %d' % (request.META.get('REMOTE_ADDR'), request.META.get('HTTP_X_REMOTE_ADDR', '0.0.0.0'), detector_info[0].device_id, 're_register', 0 if not data else 1)
        fu.string2log_append_per_day(content=log_str, path=common.LOG_PATH + 'detector_access_log/')
...
def process_business_status(request):
    try:
        ...
        # 生成业务处置系统所需文件
        if config.const.UPLOAD_BUSINESS_DISPOSAL:
            handle_data_type = 'status(business)'
            file_dir = os.path.join(config.const.DISPOSAL_DIR, 'business_status')
...
def process_system_status(request):
    try:
        ...
        if config.const.UPLOAD_BUSINESS_DISPOSAL:
            handle_data_type = 'status(system)'
            file_dir = os.path.join(config.const.DISPOSAL_DIR, 'detector_status')
