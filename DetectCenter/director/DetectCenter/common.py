...
# 发送给业务处置系统的文件内容分隔符和文件所在目录
handle_boundary = '---------------7d81741d1803de---------------'
handle_dir = os.path.dirname(BASE_DIR) + '/DetectorManagement/businessDisposition/'
...
# 获取authKey
def get_auth_key(url=AUTH_URL):
    r_auth = requests.post(url, data={'uid': test_uid, 'password': test_pwd})
    if r_auth.status_code == status.HTTP_200_OK:
        return r_auth.json()['data']['authKey'].encode('utf-8')
    else:
        return None
