...
# 处理业务系统异步
@app.task
def task_send_business_file(logger_str, flag, device_id, header, file_path, filename, url=config.const.BUSINESS_URL):
    sender.send_business_file(logger_str, flag, device_id, header, file_path, filename, url)

@app.task
def task_send_business_data(logger_str, flag, device_id, send_data, url=config.const.BUSINESS_URL):
