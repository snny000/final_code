            ...
            <div class="pull-left margin_ddos1">
                <h4><span class="tab_color">|</span>&nbsp;&nbsp;插件部署管理
                    <div class="btn-group" data-toggle="buttons">
                        <button type="button" href='plug.php' class="btn btn-default">管理中心本地</button>
                        <button type="button" href='plug_director.php' class="btn btn-default btn-primary">指挥节点下发</button>
                    </div>
                </h4>
            </div>
        </div>
...
<script>
    ...
    function List(msgListObj){
        ...
        for (var i = 0; i < msgListObj.length; i++) {
            ...
            if(msgListObj[i].plug_status == 1 && msgListObj[i].is_plug_file_release == 0) {  // 指挥下发的插件的文件还未下载
                row.find("td:eq(3)").html("<a href=\"#\" onclick=\"downloadPlugFileFromDirector(" + msgListObj[i].id + ", 1, this);\">下载文件</a>")
            } else {
           ...
           // row.find("td:eq(5)").html(prevalenceMap[msgListObj[i].prevalence]);
            ...
            if(msgListObj[i].plug_status == 1 && msgListObj[i].is_config_file_release == 0) {  // 指挥下发的插件的文件还未下载
                row.find("td:eq(8)").html("<a href=\"#\" onclick=\"downloadPlugFileFromDirector(" + msgListObj[i].id + ", 1, this);\">下载文件</a>")
            } else {
...
//            row.find("td:eq(13)").html(operateviewhtml_new);
    ...
    function downloadPlugFileFromDirector(id, flag, obj) {
        $.ajax({
            url: "/ajax_action_detector.php?uu=director.download_plug",
            type: "post",
            data: {id: id, file_type: flag},
            success:function(data) {
                var ret = JSON.parse(data);
                console.log(data);
                if (ret['code'] == 200) {
//                    $(obj).parent().html("<a url=" + ret['msg']['path'] + " href=ajax_action_download_rename_common.php?uu=download&path=" + ret['msg']['path'] + "&rename=" + ret['msg']['name'] + ">" + ret['msg']['name'] + "</a>");
                    refresh();
                } else {
                    alert(ret['msg']);
                }
...
<script>
    ...
    function pickDetectorForward(id,device_id_list,type,plug_on_device_status){
        var param= {cacheRef:'plug_director.php'}
        var currentPage = $('#pagination .active a').text()
