...
<div id="whole-wrapper">
    ...
    <div>
        <div class="row nav_margin">
            <div class="pull-left">
                <h4><span class="tab_color">|</span>&nbsp;&nbsp;应用行为WEB过滤策略
...
<!--                <h4><span class="tab_color">|</span>&nbsp;&nbsp;应用行为DNS过滤策略</h4>-->
<!--            </div>-->
            <div class="upper-btn-group margin_ddos1 pull-left">
                <button resourceid='383' id="" type="button" class="btn btn-primary nav-btn " onClick="location='rule_net_log_director.php'"><i class="fa fa-upload">&nbsp;&nbsp;</i>通联关系上报策略</button>
                <button resourceid='385' id="" type="button" class="btn btn-primary nav_interval nav-btn" onClick="location='rule_app_behavior_director.php'"><i class="fa fa-cloud-upload">&nbsp;&nbsp;</i>应用行为上报策略</button>
                <button resourceid='387' id="" type="button" class="btn btn-primary nav_interval nav-btn"  onClick="location='rule_web_filter_director.php'"><i class="fa fa-filter">&nbsp;&nbsp;</i>应用行为WEB过滤策略</button>
                <button resourceid='389' id="" type="button" class="btn btn-primary nav_interval nav-btn active"  onClick="location='rule_dns_filter_director.php'"><i class="fa fa-fire">&nbsp;&nbsp;</i>应用行为DNS过滤策略</button>
            </div>
        </div>
        ...
        <div class="row btn-banner">
            ...
            <input id="dns" type="text" class="form-control search-input btn-interval" placeholder="DNS信息（模糊搜索）">
    ...
    <div class="modal-dialog" style="width: 390px">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div>
                    <span>DNS信息（最大64字节）：</span> <input id="add_dns" maxlength="64" class="form-control";>
...
<script>
    buildFrame("menu-rule6");
    var global_policy_type=16;
...
$("#add-submit").click(function(){
    ...
    function List(msgListObj){
        ...
        for (var i = 0; i < msgListObj.length; i++) {
			...
			row.find("td:eq(2)").html(msgListObj[i].dns);
			row.find("td:eq(3)").html(ruleMap[msgListObj[i].rule_type]);
			row.find("td:eq(4)").html(matchMap[msgListObj[i].match_type]);
			// row.find("td:eq(5)").html(statusMap[msgListObj[i].rule_status]+"("+operateMap[msgListObj[i].operate]+")");
            row.find("td:eq(5)").html(ruleStatusFormat(msgListObj[i]));      //
