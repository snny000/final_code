...
<div id="whole-wrapper">
    ...
    <div>
        <div class="row nav_margin">
            <div class="pull-left">
                <h4><span class="tab_color">|</span>&nbsp;&nbsp;图片筛选策略
...
<!--                <h4><span class="tab_color">|</span>&nbsp;&nbsp;图片筛选策略</h4>-->
<!--            </div>-->
            <div class="upper-btn-group margin_ddos1 pull-left">
                <button resourceid="367" id="" type="button" class="btn btn-primary nav-btn " onClick="location='rule_keyword_file_director.php'"><i class="fa fa-key">&nbsp;&nbsp;</i>关键字检测策略</button>
                <button resourceid="369" id="" type="button" class="btn btn-primary nav_interval nav-btn" onClick="location='rule_encryption_file_director.php'"><i class="fa fa-lock">&nbsp;&nbsp;</i>加密文件检测策略</button>
                <button resourceid="371" id="" type="button" class="btn btn-primary nav_interval nav-btn"  onClick="location='rule_compress_file_director.php'"><i class="fa fa-compress">&nbsp;&nbsp;</i>多层压缩检测策略</button>
                <button resourceid="373" id="" type="button" class="btn btn-primary nav_interval nav-btn active"  onClick="location='rule_picture_file_director.php'"><i class="fa fa-file-image-o">&nbsp;&nbsp;</i>图片筛选策略</button>
            </div>
        </div>
        ...
        <div class="row btn-banner">
            ...
            <input id="filesize_minsize" type="text" class="form-control search-input btn-interval" placeholder="图片最小值">
			<input id="filesize_maxsize" type="text" class="form-control search-input btn-interval" placeholder="图片最大值">
    ...
    <div class="modal-dialog" style="width: 390px">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div>
                    <span>图片最小值（数值型，KB）:</span> <input id="add_filesize_minsize" type="number" min="0"  class="form-control";>
                    <div style="color:red"></div>
                    <span>图片最大值（数值型，KB）:</span> <input id="add_filesize_maxsize" type="number" min="0"  class="form-control";>
                    <div style="color:red"></div>
                </div>
...
<script>
    ...
    var global_policy_type = 8;


    // 设置规则提示
    $('#addModal h4').html('添加策略' + helper_ele);
    $('#addModal h4 i.hint-helper').attr("value", rule_picture_helper_text);
    var option = {
...
$("#add-submit").click(function(){
        ...
        var param = {
            ...
            "filesize_minsize":$("#add_filesize_minsize").val(),
            "filesize_maxsize":$("#add_filesize_maxsize").val(),
            ...
            var ischeck = true
            var add_filesize_minsize = $("#add_filesize_minsize").val()
            var add_filesize_maxsize = $("#add_filesize_maxsize").val()
            var add_risk = $("#add_risk").attr("value")
            if(add_filesize_minsize == ""){
                $("#add_filesize_minsize").next("div").html("图片最小值不能为空")
                ischeck = false
            }else{
                $("#add_filesize_minsize").next("div").html("")
            }
/*            var re = /^[0-9]*[1-9][0-9]*$/
            if(!(re.test(add_filesize_minsize))){
                $("#add_min_match_count").next("div").html("图片最小值要求整数")
                ischeck = false
            }else{
                $("#add_min_match_count").next("div").html("")
            }*/

            if(add_filesize_maxsize == ""){
                $("#add_filesize_maxsize").next("div").html("图片最大值不能为空")
                ischeck = false
            }else{
                $("#add_filesize_maxsize").next("div").html("")
            }
           if(add_risk == -1){
    ...
    function List(msgListObj){
        ...
        for (var i = 0; i < msgListObj.length; i++) {
			...
			row.find("td:eq(2)").html(msgListObj[i].filesize_minsize);
			row.find("td:eq(3)").html(msgListObj[i].filesize_maxsize);
            ...
            row.find("td:eq(6)").html(msgListObj[i].label);
            row.find("td:eq(7)").html(msgListObj[i].create_t);
...
//            var operatehtml=generateoperatehtml(msgListObj[i].id,msgListObj[i].device_id_list)
            row.find("td:eq(8)").html(operateviewhtml);
...
//                row.find("td:eq(10)").addClass('hasTaskGroup');
//
//            }
//            row.find("td:eq(10)").html(task_group_dom);
            row.find("td:eq(9)").html(msgListObj[i].task_id != 0 ? msgListObj[i].task_id : '');
            row.show();
   ...
   var globalSearchParam = {random:1}
    ...
    $("#searchButton").click(function(){
        ...
        var filesize_minsize =  $("#filesize_minsize").val();
		var filesize_maxsize =  $("#filesize_maxsize").val();
		var label =  $("#label").val();
        ...
        if(filesize_minsize!=""){
            globalSearchParam["filesize_minsize"] = filesize_minsize
        }
		if(filesize_maxsize!=""){
            globalSearchParam["filesize_maxsize"] = filesize_maxsize
        }
    ...
    $("#clearButton").click(function(){
        ...
        $("#rule_id").val("");
        $("#filesize_minsize").val("");
		$("#filesize_maxsize").val("");
		$("#label").val("");
		$("#device_id").val("");
 ...
 function alterSearchForm (){
        ...
        oneInput("rule_id");
        oneInput("filesize_minsize");
        oneInput("filesize_maxsize");
		oneInput("x_maxsize");
		oneInput("x_minsize");
		oneInput("y_maxsize");
		oneInput("y_minsize");
