    ...
    <div>
        ...
        <div class="row">
            <div class="upper-btn-group margin_ddos1 pull-left">
                <button resourceid="359" id="" type="button" class="btn btn-primary nav-btn active" onClick="location='rule_trojan.php'"><i class="fa fa-certificate">&nbsp;&nbsp;</i>木马攻击检测策略</button>
                <button resourceid="361" id="" type="button" class="btn btn-primary nav_interval nav-btn" onClick="location='rule_attack.php'"><i class="fa fa-bug">&nbsp;&nbsp;</i>漏洞利用检测策略</button>
    ...
    <div class="modal-dialog" style="width: 390px">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div>
                    <span>木马编号（数值型）:</span> <input id="add_trojan_id" type="number" min="0"  class="form-control";>
                    <div style="color:red"></div>
                    <span>木马名称（最长128字节）:</span> <input id="add_trojan_name"  maxlength="128" class="form-control";>
                    <div style="color:red"></div>
                </div>
                ...
                <div class="dropdown-inline">
                    ...
                    <div class="dropdown">
                        <button type="button" data-toggle="dropdown"
                                ...
                                aria-haspopup="true"
                                aria-expanded="false">
							<span id="add_trojan_type" class="pull-left" value="-1">未选择</span>
                            <i class="fa fa-sort-down pull-right"></i>
                        </button>
                        <ul class="dropdown-menu dropdown-menu-width dropdown-list-style" id="ipslist">
                ...
                <div>
                    ...
                    <div style="color:red"></div>
                    <span>攻击告警描述（可选，最长512字节）:</span> <input id="add_desc"  maxlength="128" class="form-control";>
                    <div style="color:red"></div>
                </div>
...
<script>
    ...
    $("#add-submit").click(function(){
        ...
        var param = {
            "trojan_id":$("#add_trojan_id").val(),
            "trojan_name":$("#add_trojan_name").val(),
            "trojan_type": parseInt($("#add_trojan_type").attr("value")),
//            "prevalence": parseInt($("#add_prevalence").attr("value")),
            "os": $("#add_os").val(),
            "desc": $("#add_desc").val(),
            "rule": $("#add_rule").val(),
            ...
            var ischeck = true
            var add_trojan_id = $("#add_trojan_id").val()
            var add_trojan_name = $("#add_trojan_name").val()
            var add_prevalence = $("#add_prevalence").attr("value")
            var add_os = $("#add_os").val()
            var add_desc = $("#add_desc").val()
            var add_rule = $("#add_rule").val()
			var add_trojan_type = $("#add_trojan_type").attr("value")
			var add_store_pcap = $("#add_store_pcap").attr("value")
			var add_risk = $("#add_risk").attr("value")
	

            if(add_trojan_id == ""){
                $("#add_trojan_id").next("div").html("木马编号不能为空")
                ischeck = false
            }else{
                $("#add_trojan_id").next("div").html("")
            }

            if(add_trojan_name == ""){
                $("#add_trojan_name").next("div").html("木马名称不能为空")
                ischeck = false
            }else{
                $("#add_trojan_name").next("div").html("")
            }

/*        if(add_os == ""){
            $("#add_os").next("div").html("攻击适用系统不能为空")
            ischeck = false
        }else{
            $("#add_os").next("div").html("")
        }

        if(add_desc == ""){
            $("#add_desc").next("div").html("攻击告警描述不能为空")
            ischeck = false
        }else{
            $("#add_desc").next("div").html("")
        }*/

			if(add_trojan_type == -1){
			
				$("#add_trojan_type").parent().parent().next("div").html("请选择木马类型")
				ischeck = false
			}else{
				$("#add_trojan_type").parent().parent().next("div").html("")
			}
			if(add_store_pcap == -1){
				$("#add_store_pcap").parent().parent().next("div").html("请选择是否保留")
				ischeck = false
			}else{
