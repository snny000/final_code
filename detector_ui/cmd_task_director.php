            ...
            <div class="pull-left margin_ddos1">
                <h4><span class="tab_color">|</span>&nbsp;&nbsp;命令任务
                    <div class="btn-group" data-toggle="buttons">
                        <button type="button" href='cmd_task.php' class="btn btn-default">管理中心本地</button>
                        <button type="button" href='cmd_task_director.php' class="btn btn-default btn-primary">指挥节点下发</button>
                    </div>
                </h4>
            </div>
...
//            row.find("td:eq(8)").html(formatterOperate(msgListObj[i]));
            ...
            row.find("td:eq(8)").html(result_str);

            if (msgListObj[i].save_path == '') {
                row.find("td:eq(9)").html('无附件');
            } else {
                row.find("td:eq(9)").html("<a url="+msgListObj[i].save_path+" href=ajax_action_download_rename_common.php?uu=download&path="+msgListObj[i].save_path+"&rename="+msgListObj[i].filename+">"+msgListObj[i].filename+"</a>");
            }

            row.find("td:eq(10)").html(JSON.stringify(param));
            row.show();
