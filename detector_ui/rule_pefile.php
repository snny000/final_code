...
<div id="whole-wrapper">
    ...
    <div>
        <div class="row nav_margin">
            <div class="pull-left">
                <h4><span class="tab_color">|</span>&nbsp;&nbsp;恶意程序检测策略
        ...
        <div class="row">
            <div class="upper-btn-group margin_ddos1 pull-left">
                ...
                <button resourceid="363" id="" type="button" class="btn btn-primary nav_interval nav-btn active" onClick="location='rule_pefile.php'"><i class="fa fa-user-secret">&nbsp;&nbsp;</i>恶意程序检测策略</button>
            </div>
        ...
        <div class="row btn-banner">
            ...
            <input id="malware_type" type="text" class="form-control search-input btn-interval" placeholder="恶意程序种类（模糊搜索）">
			<input id="malware_name" type="text" class="form-control search-input btn-interval" placeholder="恶意程序名称（模糊搜索）">
        ...
        <div class="row common_margin">
            <table id="maintable" class="table table-hover tbl_font_size "
                ...
                <tr >
                    ...
                    <th width="8%">规则编号</th>
                    <th width="8%">恶意程序种类</th>
                    <th width="8%">恶意程序名称</th>
					<th width="8%">检测文件的md5</th>
                    <th width="8%">十六进制字符串</th>
                    <th width="8%">告警级别</th>
                    <th width="8%">规则状态</th>
					<th width="8%">标签</th>
                    <th width="9%">创建时间</th>
    ...
    <div class="modal-dialog" style="width: 390px">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div>
                    <span>恶意程序种类（最长128字节）:</span> <input id="add_malware_type"  maxlength="128" class="form-control";>
                    <div style="color:red"></div>
                    <span>恶意程序名称（最长128字节）:</span> <input id="add_malware_name"  maxlength="128" class="form-control";>
                    <div style="color:red"></div>
                    <span>检测文件的md5（可选，最长128字节）:</span> <input id="add_md5"   maxlength="128" class="form-control";>
                    <div style="color:red"></div>
					<span>十六进制字符串（可选，最长2048字节）:</span> <input id="add_signature"  maxlength="2048" class="form-control";>
                    <div style="color:red"></div>
                </div>
...
$("#add-submit").click(function(){
        ...
        var param = {
            ...
            "malware_name":$("#add_malware_name").val(),
            "malware_type":$("#add_malware_type").val(),
            "md5":$("#add_md5").val(),
            ...
            var ischeck = true
            var add_malware_name = $("#add_malware_name").val()
            var add_malware_type = $("#add_malware_type").val()
            var add_md5 = $("#add_md5").val()
            var add_signature = $("#add_signature").val()
			var add_risk = $("#add_risk").attr("value")
			
            if(add_malware_name == ""){
                $("#add_malware_name").next("div").html("恶意程序名称不能为空")
                ischeck = false
            }else{
                $("#add_malware_name").next("div").html("")
            }

            if(add_malware_type == ""){
                $("#add_malware_type").next("div").html("恶意程序种类不能为空")
                ischeck = false
            }else{
                $("#add_malware_type").next("div").html("")
            }

            if(add_md5 == "" && add_signature == ""){
                $("#add_md5").next("div").html("检测文件的md5不能为空")
                ischeck = false
            }else{
                $("#add_md5").next("div").html("")
            }
            ...
            if(add_md5 == "" && add_signature == ""){
                $("#add_signature").next("div").html("十六进制字符串不能为空")
                ischeck = false
            }else{
    ...
    function List(msgListObj){
        ...
        for (var i = 0; i < msgListObj.length; i++) {
			...
			row.find("td:eq(2)").html(msgListObj[i].malware_type);
			row.find("td:eq(3)").html(msgListObj[i].malware_name);
			row.find("td:eq(4)").html(msgListObj[i].md5);
			row.find("td:eq(5)").html(msgListObj[i].signature);
...
,
   ...
   var globalSearchParam = {random:1}
    ...
    $("#searchButton").click(function(){
        ...
        var malware_type =  $("#malware_type").val();
		var malware_name =  $("#malware_name").val();
        ...
        if(malware_type!=""){
            globalSearchParam["malware_type"] = malware_type
        }
		if(malware_name!=""){
            globalSearchParam["malware_name"] = malware_name
        }
        if(device_id!=""){
    ...
    $("#clearButton").click(function(){
	 ...
	 function alterSearchForm (){
        ...
        oneInput("rule_id");
        oneInput("malware_name");
        oneInput("malware_type");
        oneInput("label");
		oneInput("device_id");
