    ...
    <div>
        <div class="row">
            <div class="pull-left margin_ddos1">
                <h4><span class="tab_color">|</span>&nbsp;&nbsp;检测器本地审计</h4>
            </div>

        </div>

        <div class="row btn-banner">
            <input id="device_id" type="text" class="form-control search-input date_div" placeholder="检测器编号(模糊搜索)">
			<input id="log_id" type="text" class="form-control search-input date_div btn-interval" placeholder="日志编号(模糊搜索)">
...
*/
    ...
    function List(msgListObj){
        ...
        for (var i = 0; i < msgListObj.length; i++) {
			...
			row.find("td:eq(2)").text(msgListObj[i].log_id);
			row.find("td:eq(3)").text(msgListObj[i].user);
			row.find("td:eq(4)").text(msgListObj[i].opt_type);
            row.find("td:eq(5)").html(msgListObj[i].event_type);
            row.find("td:eq(6)").html(msgListObj[i].message);
    ...
    function LoadPage(currentPage,searchParam){
        $.ajax({
            url: "/ajax_action_detector.php?uu=audit_system.count",
            type: "post",
            data: searchParam,
            success:function(data) {
                var ret = JSON.parse(data);
                ...
                pagination(ret,"/ajax_action_detector.php?uu=audit_system.show&p_size="+p_size,parseInt(currentPage),searchParam)
            },
            beforeSend: function () {
