...
<div id="whole-wrapper">
    ...
    <div>
        <div class="row nav_margin">
            <div class="pull-left">
                <h4><span class="tab_color">|</span>&nbsp;&nbsp;漏洞利用检测策略
        ...
        <div class="row">
            <div class="upper-btn-group margin_ddos1 pull-left">
                <button resourceid="359" id="" type="button" class="btn btn-primary nav-btn" onClick="location='rule_trojan.php'"><i class="fa fa-certificate">&nbsp;&nbsp;</i>木马攻击检测策略</button>
                <button resourceid="361" id="" type="button" class="btn btn-primary nav_interval nav-btn active" onClick="location='rule_attack.php'"><i class="fa fa-bug">&nbsp;&nbsp;</i>漏洞利用检测策略</button>
                <button resourceid="363" id="" type="button" class="btn btn-primary nav_interval nav-btn" onClick="location='rule_pefile.php'"><i class="fa fa-user-secret">&nbsp;&nbsp;</i>恶意程序检测策略</button>
            </div>
        ...
        <div class="row btn-banner">
            ...
            <input id="rule" type="text" class="form-control search-input btn-interval" placeholder="规则内容（模糊搜索）">
        ...
        <?php
            ...
            <div class="dropdown btn-interval dropdown-inline">
                <button type="button" data-toggle="dropdown"
                        ...
                        aria-haspopup="true"
                        aria-expanded="false">
                    <span id="attack_type" class="pull-left" value="-1">所有攻击类型</span>
                    <i class="fa fa-sort-down pull-right"></i>
                </button>
                <ul class="dropdown-menu dropdown-menu-width dropdown-list-style" id="ipslist">
                    <li onclick="selectProtoFwd(this);" value="-1">所有攻击类型</li>
                    <li onclick="selectProtoFwd(this);" value="1">http</li>
                    <li onclick="selectProtoFwd(this);" value="2">RPC</li>
                    <li onclick="selectProtoFwd(this);" value="3">WEBCGI</li>
                    <li onclick="selectProtoFwd(this);" value="4">overflow</li>
                    <li onclick="selectProtoFwd(this);" value="5">systemflaw</li>
                </ul>
            </div>
            &nbsp;
    ...
    <div class="modal-dialog" style="width: 390px">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="dropdown-inline">
                    ...
                    <div class="dropdown">
                        <button type="button" data-toggle="dropdown"
                                ...
                                aria-haspopup="true"
                                aria-expanded="false">
							<span id="add_store_pcap" class="pull-left" value="-1">未选择</span>
                            <i class="fa fa-sort-down pull-right"></i>
                        </button>
                        <ul class="dropdown-menu dropdown-menu-width dropdown-list-style" id="ipslist">
                ...
                <div class="dropdown-inline">
                    ...
                    <div class="dropdown">
                        <button type="button" data-toggle="dropdown"
                                ...
                                aria-haspopup="true"
                                aria-expanded="false">
                            <span id="add_attack_type" class="pull-left" value="-1">未选择</span>
                            <i class="fa fa-sort-down pull-right"></i>
                        </button>
                        <ul class="dropdown-menu dropdown-menu-width dropdown-list-style" id="ipslist">
                            <li onclick="selectProtoFwd(this);" value="-1">未选择</li>
                ...
                <div>
                    <span>攻击适用程序（可选，最长128字节）:</span> <input id="add_application" maxlength="128" class="form-control";>
                    <div style="color:red"></div>
                    <span>攻击适用系统（可选，最长128字节）:</span> <input id="add_os"  maxlength="128" class="form-control";>
                    <div style="color:red"></div>
                </div>
                ...
                <div>
                    <span> 规则内容（最长2048字节）:</span>
                    <textarea id="add_rule" style="width:100%;height:80px;resize: vertical;"   maxlength="2048" class="form-control"></textarea>
                    <div style="color:red"></div>
                </div>
                <div>
                    <div> 选择任务组:</div>
...
<script>
    ...
    $("#add-submit").click(function(){
        ...
        var param = {
            "store_pcap":parseInt($("#add_store_pcap").attr("value")),
            "attack_type": parseInt($("#add_attack_type").attr("value")),
            "application": $("#add_application").val(),
            "os": $("#add_os").val(),
            ...
            var ischeck = true
            var add_store_pcap = $("#add_store_pcap").attr("value")
            var add_application = $("#add_application").val()
            var add_os = $("#add_os").val()
            var add_rule = $("#add_rule").val()
			var add_attack_type = $("#add_attack_type").attr("value")

			var add_risk = $("#add_risk").attr("value")


			if(add_attack_type == -1){
				$("#add_attack_type").parent().parent().next("div").html("请选择攻击类型")
				ischeck = false
			}else{
				$("#add_attack_type").parent().parent().next("div").html("")
			}
			if(add_store_pcap == -1){
				$("#add_store_pcap").parent().parent().next("div").html("请选择是否保留报文")
				ischeck = false
			}else{
				$("#add_store_pcap").parent().parent().next("div").html("")
			}
			if(add_risk == -1){
            ...
            if(add_rule == ""){
                $("#add_rule").next("div").html("规则内容不能为空")
                ischeck = false
            }else{
                $("#add_rule").next("div").html("")
            }
    ...
    function List(msgListObj){
        ...
        for (var i = 0; i < msgListObj.length; i++) {
            ...
            row.find("td:eq(2)").html(store_pcapMap[msgListObj[i].store_pcap]);
			row.find("td:eq(3)").html(msgListObj[i].rule);
            row.find("td:eq(4)").html(attack_typeMap[msgListObj[i].attack_type]);
...
,
    ...
    //第一次加载分页
    //LoadPage(1,globalSearchParam,0)
    globalSearchParam = cacheSearchParam;
    LoadPage(cachePage,globalSearchParam,0);
    alterSearchForm()
    ...
    $("#searchButton").click(function(){
        ...
        var attack_type = $("#attack_type").attr("value").toString();
        ...
        if(attack_type!="-1"){
            globalSearchParam["attack_type"] = attack_type
        }
