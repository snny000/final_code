...
<div id="whole-wrapper">
    ...
    <div>
        <div class="row nav_margin">
            <div class="pull-left">
                <h4><span class="tab_color">|</span>&nbsp;&nbsp;应用行为上报策略
...
<!--                <h4><span class="tab_color">|</span>&nbsp;&nbsp;应用行为上报策略</h4>-->
<!--            </div>-->
            <div class="upper-btn-group margin_ddos1 pull-left">
                ...
                <button resourceid='385' id="" type="button" class="btn btn-primary nav_interval nav-btn active" onClick="location='rule_app_behavior_director.php'"><i class="fa fa-cloud-upload">&nbsp;&nbsp;</i>应用行为上报策略</button>
        ...
        <div class="row btn-banner">
            <input id="interval" type="text" class="form-control search-input" placeholder="每次上报的间隔">
			<input id="num" type="text" class="form-control search-input btn-interval" placeholder="缓存事件数">
        ...
        <div class="row common_margin">
            <table id="maintable" class="table table-hover tbl_font_size "
                ...
                <tr >
                    ...
                    <th width="11%">规则编号</th>
                    <th width="11%">每次上报的间隔</th>
                    <th width="11%">缓存事件数</th>
                    <th width="11%">规则状态</th>
					<th width="11%">标签</th>
                    <th width="11%">创建时间</th>
                    <th width="11%">生效范围</th>
    ...
    <div class="modal-dialog" style="width: 390px">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div>
                     <span>每次上报的时间间隔（数值型，秒）:</span> <input id="add_interval" type="number" min="0"  class="form-control";>
                    <div style="color:red"></div>
                    <span>缓存事件数（数值型）:</span> <input id="add_num" type="number" min="0"  class="form-control";>
                    <div style="color:red"></div>
                </div>
                <div>
                    <div> 选择任务组:</div>
...
$("#add-submit").click(function(){
        ...
        var param = {
            ...
            var ischeck = true
            var add_interval = $("#add_interval").val()
            var add_num = $("#add_num").val()
            
			
            if(add_interval == ""){
                $("#add_interval").next("div").html("每次上报的间隔不能为空")
                ischeck = false
            }else{
                $("#add_interval").next("div").html("")
            }

            if(add_num == ""){
                $("#add_num").next("div").html("缓存事件数不能为空")
                ischeck = false
            }else{
                $("#add_num").next("div").html("")
            }
            if(!ischeck){
                return;
            }
    ...
    function List(msgListObj){
        ...
        for (var i = 0; i < msgListObj.length; i++) {
			...
			row.find("td:eq(2)").html(msgListObj[i].interval);
			row.find("td:eq(3)").html(msgListObj[i].num);
            ...
            row.find("td:eq(5)").html(msgListObj[i].label);
...
//            row.find("td:eq(9)").html(task_group_dom);
    ...
    $("#searchButton").click(function(){
        ...
        if(interval!=""){
            globalSearchParam["interval"] = interval
        }
		if(num!=""){
            globalSearchParam["num"] = num
        }
		if(device_id!=""){
