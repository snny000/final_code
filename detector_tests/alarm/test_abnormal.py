...
def send_abnormal(session, detector_id, alarm_id):
    headers = {
    ...
    abnormal_data = {
        ...
        "alert_type": random.randint(1, 4),
        "alert_policy": "HTTP隧道传输、持续活动",
        "alert_desc": "关键上下文",
    }

    r = session.post(ROOT_URL + "V1/abnormal/inner_policy",
                     data=json.dumps(abnormal_data), headers=headers,
...
def send_abnormal_file(session, detector_id, alarm_id, num):
    file_name = str(int(time.time())) + '_002_' + str(num) + '.pcap'
    file_path = TEST_PATH + 'abnormal/' + detector_id + '/'
    ...
    r = session.post(ROOT_URL + "V1/abnormal/inner_policy_file",
...
if __name__ == '__main__':
    ...
    # alert_id = '20170525000005'
    alert_id = sys.argv[1]
    file_num = 1
    s = requests.Session()
    send_abnormal(s, device_id, alert_id)
    send_abnormal_file(s, device_id, alert_id, file_num)
