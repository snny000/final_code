...
def send_ip_listen_data(session, detector_id, alarm_id):
    headers = {
    ...
    send_url = ROOT_URL + "V1/object_listen/ip_listen/center_policy"
    print send_url
...
def send_ip_listen_file(session, detector_id, alarm_id, num):
    file_name = str(int(time.time())) + '_002_' + str(num) + '.pcap'
    file_path = TEST_PATH + 'intercept_ip/' + detector_id + '/'
    ...
    send_url = ROOT_URL + "V1/object_listen/ip_listen/center_policy_pcap"
    print send_url
...
if __name__ == '__main__':
    ...
    n = 1
    s = requests.Session()
    send_ip_listen_data(s, device_id, a_id)
    send_ip_listen_file(s, device_id, a_id, n)
